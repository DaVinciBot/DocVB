...
Objectif: installer Cinny Desktop, se connecter au serveur privé chat.davincibot.fr et créer un compte avec un jeton
d'inscription (token privé).
...
## Créer un compte avec un token privé (registration token) {/*#créer-un-compte-avec-un-token-privé*/}
...
- Identifiant: @votre_utilisateur:chat.davincibot.fr ou juste votre_utilisateur si Cinny est déjà pointé sur
  chat.davincibot.fr.
...
:::note
Le jeton d'inscription est confidentiel. Ne l'intégrez pas dans ce document et évitez toute capture d'écran
montrant ce code.
:::
